 ...
 * @return array
 */
function trickleup_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! trickleup_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'trickleup_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'trickleup_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'trickleup' ) . $network['name'],
 ...
 * @return string
 */
function trickleup_get_single_share_buttons() {
	ob_start();
