...
if ( ! class_exists( 'Trickleup_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Trickleup_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'trickleup' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'trickleup' );
			$this->widget_id          = 'trickleup-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'trickleup' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'trickleup' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'trickleup' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'trickleup' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'trickleup' ),
						),
					),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'trickleup' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'trickleup_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'trickleup_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'trickleup' ),
				...
				echo apply_filters( 'trickleup_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'trickleup_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function trickleup_register_taxonomy_tiles_widget() {
		register_widget( 'Trickleup_Taxonomy_Tiles_Widget' );
	}
}
