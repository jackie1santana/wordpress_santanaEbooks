...
if ( ! class_exists( 'Trickleup_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Trickleup_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'trickleup' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'trickleup' );
			$this->widget_id          = 'trickleup_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => TRICKLEUP_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'trickleup' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'trickleup' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'trickleup' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'trickleup' ),
						'categories' => esc_html__( 'Show categories', 'trickleup' ),
						'tags'       => esc_html__( 'Show tags', 'trickleup' ),
						'author'     => esc_html__( 'Show author', 'trickleup' ),
						'date'       => esc_html__( 'Show date', 'trickleup' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'trickleup' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'trickleup' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template(  apply_filters( 'trickleup_featured_posts_block_widget_view_dir', 'inc/widgets/featured-posts-block/views/item.php' ), false, false );
			 ...
			 */
			$query = apply_filters( 'trickleup_featured_posts_block_query', $query, $this->instance );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'trickleup' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'trickleup' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'trickleup' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'trickleup' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'trickleup' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'trickleup_register_featured_posts_block_widget' );

	if ( false === function_exists( 'trickleup_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function trickleup_register_featured_posts_block_widget() {
			register_widget( 'Trickleup_Featured_Posts_Block_Widget' );
		}
	}
}
