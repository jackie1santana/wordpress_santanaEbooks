 ...
 * @package Trickleup
 */
if ( ! class_exists( 'Trickleup_Carousel_Widget' ) ) {

	/**
	 * Class Trickleup_Carousel_Widget.
	 */
	class Trickleup_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'trickleup' );
			$this->widget_id          = 'trickleup_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'trickleup' );
			...
			$this->settings           = array(
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 5,
					'label'     => esc_html__( 'Number of slides per view', 'trickleup' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'trickleup' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'trickleup' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 30,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'trickleup' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'trickleup' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'trickleup' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'trickleup' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'trickleup_carousel_widget_view_dir', 'inc/widgets/carousel/views/carousel-view.php' ), false, false );
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', trickleup_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="trickleup-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'trickleup' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'trickleup_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'trickleup' ) . '</h5>';
			}
	...
	add_action( 'widgets_init', 'trickleup_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function trickleup_register_carousel_widget() {
		register_widget( 'Trickleup_Carousel_Widget' );
	}
}
