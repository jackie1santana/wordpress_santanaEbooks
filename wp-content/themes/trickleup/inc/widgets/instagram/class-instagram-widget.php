...
if ( ! class_exists( 'Trickleup_Instagram_Widget' ) ) {

	/**
	 * Class Trickleup_Instagram_Widget.
	 */
	class Trickleup_Instagram_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'trickleup' );
			$this->widget_id          = 'trickleup_widget_instagram';
			$this->widget_name        = esc_html__( 'Instagram', 'trickleup' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'Instagram', 'trickleup' ),
					'label' => esc_html__( 'Title', 'trickleup' ),
				),
				'tag' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'trickleup' ),
				),
				'image_counter' => array(
					'type'       => 'stepper',
					'value'      => '6',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Number of photos', 'trickleup' ),
				),
				'display_caption' => array(
					...
					'options' => array(
						'display_caption_check' => esc_html__( 'Caption', 'trickleup' ),
					),
				),
				'display_date' => array(
					'type'  => 'checkbox',
					...
					'options' => array(
						'display_date_check' => esc_html__( 'Date', 'trickleup' ),
					),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'trickleup' ) . $args['after_widget'];
			}
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'trickleup' ) . $args['after_widget'];
			}

			$this->widget_start( $args, $instance );

			printf( '<div class="%s">',
				join( ' ', apply_filters( 'trickleup_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
			);
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'trickleup_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						$photo['caption'],
						apply_filters( 'trickleup_instagram_widget_caption_length', 10 ),
						apply_filters( 'trickleup_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_image( $photo ) {
			...
			$width  = apply_filters( 'trickleup_instagram_image_size', 150, 'width' );
			$height = apply_filters( 'trickleup_instagram_image_size', 150, 'height' );
		...
		public function get_tags_url() {
			return apply_filters( 'trickleup_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		...
		public function get_post_url() {
			return apply_filters( 'trickleup_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
	...
	add_action( 'widgets_init', 'trickleup_register_instagram_widget' );
	/**
	 * Register instagram widget.
	 */
	function trickleup_register_instagram_widget() {
		register_widget( 'Trickleup_Instagram_Widget' );
	}
}
