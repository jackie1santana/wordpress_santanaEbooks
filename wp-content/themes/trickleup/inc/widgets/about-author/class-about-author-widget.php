 ...
 * @package Trickleup
 */
if ( ! class_exists( 'Trickleup_About_Author_Widget' ) ) {

	/**
	 * Class Trickleup_About_Author_Widget.
	 */
	class Trickleup_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'trickleup widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'trickleup' );
			$this->widget_id          = 'trickleup_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'trickleup' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'trickleup' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'trickleup' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'trickleup' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'trickleup' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'trickleup' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'trickleup' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'trickleup' ),
					'label' => esc_html__( 'Link label', 'trickleup' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'trickleup' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'trickleup-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'trickleup_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'trickleup_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function trickleup_register_about_author_widgets() {
	register_widget( 'Trickleup_About_Author_Widget' );
}
