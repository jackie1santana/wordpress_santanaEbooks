 ...
 * @package Trickleup
 */
if ( ! class_exists( 'Trickleup_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Trickleup_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'trickleup' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'trickleup' );
			...
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => 'Image Grid',
					'label' => esc_html__( 'Widget title', 'trickleup' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'trickleup' ),
				...
				'tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'trickleup' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'trickleup' ),
						'title'         => esc_html__( 'Post Title', 'trickleup' ),
						'comment_count' => esc_html__( 'Comment Count', 'trickleup' ),
					),
					'label'   => esc_html__( 'Post sorted', 'trickleup' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'trickleup' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'trickleup' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'trickleup' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'trickleup_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'trickleup_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'trickleup_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'trickleup_image_grid_widget_before',
				...
				echo apply_filters( 'trickleup_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'trickleup_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function trickleup_register_image_grid_widget() {
		register_widget( 'Trickleup_Image_Grid_Widget' );
	}
}
