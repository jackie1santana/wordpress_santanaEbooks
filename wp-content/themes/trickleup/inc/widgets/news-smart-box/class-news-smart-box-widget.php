...
/**
 * Class Trickleup_News_Smart_Box_Widget
 */
class Trickleup_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'trickleup_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'trickleup' );
		$this->widget_description = esc_html__( 'News Smart Box', 'trickleup' );
		$this->utility            = trickleup_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'trickleup' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'trickleup' ),
						'img_src' => TRICKLEUP_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'trickleup' ),
						'img_src' => TRICKLEUP_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'trickleup' ),
						'img_src' => TRICKLEUP_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'trickleup' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'trickleup' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'trickleup' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'trickleup' ),
				'placeholder'      => esc_html__( 'Select tag', 'trickleup' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'trickleup' ),
				'multiple'         => true,
			...
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'trickleup' ),
			),
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'trickleup' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'trickleup' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'trickleup' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'trickleup' ),
			),
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'trickleup_news_smart_box_default_query_args', array(
			'posts_per_page' => '5',
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'trickleup' ) . '</h4></div>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$author = $this->utility->meta_data->get_author(array(
				...
				'prefix'  => esc_html__( 'by ', 'trickleup' ),
			...
			$comments = $this->utility->meta_data->get_comment_count(array(
				...
				'sufix'   => get_comments_number_text( esc_html__( 'No comment(s)', 'trickleup' ), esc_html__( '1 comment', 'trickleup' ), esc_html__( '% comments', 'trickleup' ) ),
...
add_action( 'widgets_init', 'trickleup_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function trickleup_register_news_smart_box_widgets() {
	register_widget( 'Trickleup_News_Smart_Box_Widget' );
}
