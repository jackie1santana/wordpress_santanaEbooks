...
if ( ! class_exists( 'Trickleup_About_Widget' ) ) {

	/**
	 * Class Trickleup_About_Widget.
	 */
	class Trickleup_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'trickleup' );
			$this->widget_id          = 'trickleup_widget_about';
			$this->widget_name        = esc_html__( 'About Trickleup', 'trickleup' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'trickleup' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'trickleup' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'trickleup' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'trickleup' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'trickleup' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, trickleup_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'trickleup_widget_about_template_file', 'inc/widgets/about/views/about.php' ), false, false );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'trickleup_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'trickleup_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'trickleup_register_about_widget' );
/**
 * Register about widget.
 */
function trickleup_register_about_widget() {
	register_widget( 'Trickleup_About_Widget' );
}
