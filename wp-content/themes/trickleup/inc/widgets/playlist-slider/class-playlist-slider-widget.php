...
if ( ! class_exists( 'Trickleup_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Trickleup_Playlist_Slider_Widget.
	 */
	class Trickleup_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'trickleup' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'trickleup' );
			$this->widget_id          = apply_filters( 'trickleup_playlist_slider_widget_ID', 'trickleup_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'trickleup_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = trickleup_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'trickleup' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'trickleup' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'trickleup' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'trickleup' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'trickleup' ),
						true  => esc_html__( 'Buttons', 'trickleup' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'trickleup' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'trickleup' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'trickleup' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'trickleup' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'trickleup' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'trickleup' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'trickleup' ),
						'author'   => esc_html__( 'Author', 'trickleup' ),
						'comments' => esc_html__( 'Comment count', 'trickleup' ),
						'category' => esc_html__( 'Category', 'trickleup' ),
						'post_tag' => esc_html__( 'Tag', 'trickleup' ),
					),
					'label' => esc_html__( 'Display post meta data', 'trickleup' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'trickleup' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'trickleup_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'trickleup-thumb-s',
					'mobile_size' => 'trickleup-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'trickleup_playlist_buttons', false ),
...
add_action( 'widgets_init', 'trickleup_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function trickleup_register_playlist_slider_widget() {
	register_widget( 'Trickleup_Playlist_Slider_Widget' );
}
