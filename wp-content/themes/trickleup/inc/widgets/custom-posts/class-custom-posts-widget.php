...
if ( ! class_exists( 'Trickleup_Custom_Posts_Widget' ) ) {

	/**
	 * Class Trickleup_Custom_Posts_Widget.
	 */
	class Trickleup_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'trickleup' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'trickleup' );
			$this->widget_id          = apply_filters( 'trickleup_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'trickleup_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = trickleup_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'trickleup' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'trickleup' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'trickleup' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'trickleup' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'trickleup' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'trickleup' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'trickleup' ),
						'category'      => esc_html__( 'Category', 'trickleup' ),
						'post_tag'      => esc_html__( 'Tag', 'trickleup' ),
						'more_button'   => esc_html__( 'More Button', 'trickleup' ),
					),
				...
				'button_text' => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'trickleup' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'trickleup_custom_posts_widget_view_dir', 'inc/widgets/custom-posts/views/custom-post-view.php' ), false, false );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', trickleup_theme()->customizer->get_default( 'footer_widget_columns' ) );
	...
	add_action( 'widgets_init', 'trickleup_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function trickleup_register_custom_posts_widget() {
		register_widget( 'Trickleup_Custom_Posts_Widget' );
	}
}
