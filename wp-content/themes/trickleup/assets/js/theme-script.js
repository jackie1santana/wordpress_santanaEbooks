...
(function ($) {
	...
	CherryJsCore.theme_script = {
			...
			$('.page-template-landing div:not(.woocommerce-tabs) a[href*=#]:not([href=#])').on('click', function () {
				...
				if (location.pathname.replace(/^\//, '') === this.pathname.replace(/^\//, '') || location.hostname === this.hostname) {
					...
					if ($(window).width() <= 767) {
						...
						if (target.length) {
							$('html, body').animate({
								scrollTop: target.offset().top - menuHeight + 300
							}, 1000);
		...
		smart_slider_init: function (self) {
			$('.trickleup-smartslider').each(function () {
				var slider = $(this),
		...
		swiper_carousel_init: function (self) {
			...
			jQuery('.trickleup-carousel').each(function () {
				var swiper = null,
		...
		navbar_init: function (self) {
			...
			$(window).load(function () {
				...
				var $layout = window.trickleup.labels.header_layout,
					$navbar = ('style-3' === $layout || 'style-7' === $layout) ? $('.vertical-menu-toggle') : $('#main-menu');

				if (!$.isFunction(jQuery.fn.stickUp) || undefined === window.trickleup.stickUp || !$navbar.length) {
					return !1;
				}
		...
		subscribe_init: function (self) {
			CherryJsCore.variable.$document.on('click', '.subscribe-block__submit', function (event) {
				...
				$.ajax({
					url: trickleup.ajaxurl,
					type: 'post',
					dataType: 'json',
					data: {
						action: 'trickleup_subscribe',
						mail: mail,
						nonce: nonce
					},
					error: function () {
		...
		isMegaMenuEnabled: function () {
			...
			if (undefined === window.trickleup.megaMenu) {
				return false;
			}

			if (true === window.trickleup.megaMenu.isActive && 'main' == window.trickleup.megaMenu.location) {
				return true;
			}
		...
		main_menu: function (self, $mainNavigation) {
			...
			var transitionend = 'transitionend oTransitionEnd webkitTransitionEnd',
				...
				subMenu,
				index = -1,
				$layout = window.trickleup.labels.header_layout;

			if (hasprop.call(window, 'trickleup') &&
				hasprop.call(window.trickleup, 'more_button_options') &&
				hasprop.call(window.trickleup.more_button_options, 'more_button_type')) {
				switch (window.trickleup.more_button_options.more_button_type) {
					case 'image':
						imgurl = window.trickleup.more_button_options.more_button_image_url;
						if (window.trickleup.more_button_options.retina_more_button_image_url) {
							srcset = ' srcset="' + window.trickleup.more_button_options.retina_more_button_image_url + ' 2x"';
						}
						moreMenuContent = '<img src="' + imgurl + '"' + srcset + ' alt="' + moreMenuContent + '">';
						hasimg = true;
						break;
					case 'icon':
						moreMenuContent = '<i class="fa ' + window.trickleup.more_button_options.more_button_icon + '"></i>';
						hasicon = true;
						break;
					case 'text':
					default:
						moreMenuContent = window.trickleup.more_button_options.more_button_text || moreMenuContent;
						hasimg = false;
						hasicon = false;
						break;
				}
		...
		mobile_menu_toggle: function (self) {
		...
		mobile_menu: function (self) {
			...
			function closeMobilemenuClick() {
				// if ( 768 <= width && $menuitem.on('click') ) {
				// 	$( 'html' ).removeClass( 'mobile-menu-active' );
				// };
			}
		...
		to_top_init: function (self) {
			if ($.isFunction(jQuery.fn.UItoTop) && undefined !== window.trickleup.toTop) {
				$().UItoTop({
					text: trickleup.labels.totop_button,
					scrollSpeed: 600
				});
			}
		},
